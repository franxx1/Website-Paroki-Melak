...
onMounted(async () => {
    ...
    try {
        ...
        if (sakramenStore.sakramenItems.length === 0) {
            await sakramenStore.fetchSakramen();
        }

        await sakramenStore.fetchSakramenDetail(uid);
    } catch (err) {
        console.error("Gagal mengambil detail sakramen:", err);
...
watch(
  ...
  async (newUid) => {
    loading.value = true;
    try {
      await sakramenStore.fetchSakramenDetail(newUid);
    } finally {
      loading.value = false;
    }
  }
        ...
        <div class="container mx-auto max-w-6xl px-6 md:px-12">
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-4 gap-12">
                        ...
                        <div class="md:hidden mb-6">
                            ...
                            <div class="overflow-hidden transition-all duration-500 ease-in-out"
                                ...
                                <ul class="py-2 space-y-1 bg-[#FFF9E6]/50 backdrop-blur-sm">
                                    <li v-for="item in sakramenStore.sakramenItems" :key="item.uid">
                                        <button @click="navigateTo(item.uid)"
                                            ...
                                            :class="sakramenStore.sakramenDetail?.uid === item.uid ? 'text-[#b8860b] font-bold' : 'text-[#5D181E]'">
                                            ...
                                            <span class="text-lg transition-transform duration-300"
                                                :class="sakramenStore.sakramenDetail?.uid === item.uid ? 'translate-x-1' : 'opacity-0 group-hover:opacity-100 group-hover:translate-x-1'">
                                                &rarr;
                                            </span>
                                        </button>
                                    </li>
                        ...
                        <ul class="hidden md:block space-y-1">
                            <li v-for="item in sakramenStore.sakramenItems" :key="item.uid">
                                <button @click="navigateTo(item.uid)"
                                    ...
                                    :class="sakramenStore.sakramenDetail?.uid === item.uid ? 'text-[#b8860b] font-bold bg-[#5D181E]/5' : 'text-[#5D181E] hover:text-[#b8860b] hover:bg-[#5D181E]/5'">
                ...
                <div class="md:col-span-3">
                    <div v-if="sakramenStore.sakramenDetail" class="animate-fade-in">

                        <!-- Title & Banner -->
                        <div class="mb-8">
                            <h1
                                class="text-3xl md:text-4xl font-serif text-[#5D181E] font-bold mb-6 uppercase tracking-wide">
                                {{sakramenStore.sakramenDetail.data.nama_sakramen[0].text}}
                            </h1>

                            <div class="w-full h-64 md:h-96 rounded-xl overflow-hidden shadow-lg relative mb-8">
                                <img v-if="sakramenStore.sakramenDetail.data.gambar_sakramen.url"
                                    :src="sakramenStore.sakramenDetail.data.gambar_sakramen.url"
                        ...
                        <!-- SLICES CONTENT -->
                        <div v-if="sakramenStore.sakramenDetail.data.body" class="space-y-12">
                            <div v-for="(slice, index) in sakramenStore.sakramenDetail.data.body" :key="index">

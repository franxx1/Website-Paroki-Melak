    ...
    <section class="container mx-auto px-4 pt-32 pb-10">
        ...
        <div v-else class="max-w-3xl mx-auto">
            ...
            <h1 class="text-3xl font-bold text-[#5D181E] mb-4">
                {{
                    pengumuman.data?.judul_pengumuman?.[0]?.text || "Judul Pengumuman"
                }}
            </h1>
            ...
            <p class="text-gray-500 mb-6">
                {{ pengumuman
                    ? new Date(pengumuman.data.tanggal_rilis_pengumuman).toLocaleDateString(
                        "id-ID"
                    )
                    : "" }}
            </p>
            ...
            <div class="text-[#5D181E]/80 text-sm mb-4"
                v-html="pengumuman ? asHTML(pengumuman.data.isi_pengumuman) : ''"></div>

            <!-- File Viewer (PDF/Image) -->
            <div v-if="pengumuman?.data.pengumuman_file?.url" class="mt-8">
                <!-- PDF Viewer -->
                <div v-if="pengumuman.data.pengumuman_file.url.toLowerCase().includes('.pdf')"
                    class="w-full h-[600px] border border-gray-200 rounded-lg overflow-hidden mb-4">
                    <iframe :src="pengumuman.data.pengumuman_file.url" class="w-full h-full" frameborder="0"></iframe>
                </div>

                <!-- Image Viewer -->
                <div v-else-if="['.jpg', '.jpeg', '.png', '.gif', '.webp'].some(ext => pengumuman.data.pengumuman_file.url.toLowerCase().includes(ext))"
                    class="mb-4">
                    <img :src="pengumuman.data.pengumuman_file.url" alt="Lampiran Pengumuman"
                        class="w-full h-auto rounded-lg border border-gray-200 shadow-sm" />
                </div>

                <!-- Download Link (Always visible as fallback/option) -->
                <a :href="pengumuman.data.pengumuman_file.url" target="_blank" rel="noopener noreferrer"

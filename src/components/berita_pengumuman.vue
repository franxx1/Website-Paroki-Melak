...
onMounted(async () => {
  const [beritaRes, pengumumanRes] = await Promise.all([
    client.get({
      ...
      pageSize: 3,
      orderings: {
        field: "my.berita.tanggal-rilis-berita",
        direction: "desc",
      },
    }),
    ...
    client.get({
      ...
      pageSize: 3,
      orderings: {
        field: "my.pengumuman.tanggal_rilis_pengumuman",
        direction: "desc",
      },
    }),
  ]);
  ...
  pengumuman.value = pengumumanRes.results;
...
<template>
  <section
    class="min-h-screen font-sans text-slate-800 flex flex-col"
    ...
    >
      ...
      <div class="container mx-auto max-w-6xl relative z-10">
        ...
        <div class="flex flex-col items-center mb-16">
          <span
            class="text-[#5D181E] tracking-[0.3em] uppercase text-sm mb-2 font-bold opacity-60"
            >Paroki Katedral Jakarta</span
          >
          <h1
            class="text-4xl md:text-6xl font-serif text-[#5D181E] mb-4 relative text-center"
          >
            Berita & Pengumuman
            <span
              class="absolute -bottom-4 left-1/2 -translate-x-1/2 w-24 h-1 bg-[#5D181E] rounded-full"
            ></span>
          </h1>
        </div>
        ...
        <div v-else class="space-y-20">
          ...
          <section>
            <div class="flex items-center space-x-4 mb-8">
              <div
                class="w-10 h-10 rounded-full bg-[#5D181E] flex items-center justify-center text-[#FFF9E6]"
              >
                <svg
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              ...
              >
                ...
                <div class="h-48 bg-[#5D181E]/10 relative overflow-hidden">
                  <img
                    v-if="item.data?.['foto-berita']?.url"
                    :src="item.data['foto-berita'].url"
                    :alt="item.data['foto-berita'].alt || 'Berita Image'"
                    class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110"
                  />
                  <div
                    v-else
                    class="w-full h-full flex items-center justify-center text-[#5D181E]/30"
                  >
                ...
                <div class="p-6 flex flex-col flex-grow">
                  <span
                    class="text-xs font-bold text-[#5D181E]/60 uppercase tracking-wider mb-2"
                  >
                    {{
                      item.data?.["tanggal-rilis-berita"] ||
                  ...
                  >
                    {{ item.data?.berita?.[0]?.text || "Judul Berita" }}
                  </h3>
                  <p
                    class="text-[#5D181E]/80 text-sm line-clamp-3 mb-4 flex-grow"
                  >
                    {{
                      item.data?.["isi-berita"]?.[0]?.text ||
          ...
          <section>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              <div
                ...
                :key="index"
                class="bg-[#FFF9E6] border border-[#5D181E]/10 rounded-xl p-6 hover:bg-[#fcef91] transition-all duration-300 shadow-sm hover:shadow-md group cursor-pointer"
              >
                <div class="flex justify-between items-start mb-4">
                  ...
                  <svg
                    ...
                    stroke-width="1.5"
                    stroke="currentColor"
                    class="w-5 h-5 text-[#5D181E] opacity-50 group-hover:opacity-100"
                  >
                    <path
                      stroke-linecap="round"
                      stroke-linejoin="round"
                ...
                <h3
                  class="text-lg font-serif text-[#5D181E] font-bold mb-2 leading-snug"
                >
                  {{
                    item.data?.judul_pengumuman?.[0]?.text || "Judul Pengumuman"
                  }}
                </h3>
                <p class="text-[#5D181E]/80 text-sm line-clamp-4 mb-4">
                  {{
                    item.data?.["isi-pengumuman"]?.[0]?.text ||
                    "Klik untuk melihat detail pengumuman ini."
                  }}
                </p>

                <!-- Download/View Link if available -->
                <a
                  v-if="item.data?.['upload-pengumuman']?.url"
                  :href="item.data['upload-pengumuman'].url"
                  target="_blank"
                ...
                <div
                  class="w-full h-0.5 bg-[#5D181E]/10 group-hover:bg-[#5D181E]/30 transition-colors mt-4"
                ></div>
              </div>
            </div>
          </section>
